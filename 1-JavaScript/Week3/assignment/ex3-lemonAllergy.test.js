...
// ! Function under test
function sanitizeFruitBasket(fruitBasket, fruitToBeRemoved) {
  const filteredBasket = fruitBasket.filter(
    (fruit) => fruit !== fruitToBeRemoved
  );
...
describe('sanitizeFruitBasket', () => {
  ...
  test('should return a new array that does not include the unwanted `lemon`', () => {
    // TODO replace next line with your code
    const filteredBasket = sanitizeFruitBasket(fruitBasket, 'lemon');
    const expected = fruitBasket.filter((fruit) => fruit !== 'lemon');
    expect(filteredBasket).toEqual(expected);
  });
});
